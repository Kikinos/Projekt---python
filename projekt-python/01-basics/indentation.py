...
def vypis_delitele(cislo):
    """
    Funkce najde a vypíše všechny dělitele zadaného čísla.
    Pokud má číslo pouze 2 dělitele, upozorní že číslo je prvočíslem.
    Parametr:
        cislo: číslo, pro které se hledají dělitelé.
    """
    pocet = 0
    print(f"Dělitele čísla {cislo} jsou:")
    for i in range(1, cislo + 1):
        if cislo % i == 0:
            print(i)
            pocet += 1
    if pocet == 2:
        print("Toto číslo je prvočíslo.")

print(vypis_delitele.__doc__)
